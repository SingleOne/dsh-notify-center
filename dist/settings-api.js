import { SettingsRevisionConflictError, SettingsValidationError, } from './settings-store.js';
...
function requestOrigin(req) {
    ...
    if (!isLoopback(origin.hostname))
        throw new HttpError(403, 'loopback host required');
    return origin;
}
function validateRequestSource(req) {
    if (!isLoopback(req.socket.remoteAddress))
        throw new HttpError(403, 'loopback access only');
...
export function createSettingsApiHandler(store, logger = console) {
...
export function registerSettingsApi(webServer, store, logger = console) {
